...
int64_t DatabaseProcessor::save_pending_request(int64_t user_id,
...
void DatabaseProcessor::update_pending_status(int64_t request_id,
...
void DatabaseProcessor::schedule_retry(int64_t request_id, int minutes_delay,
                                       const std::string &error) {
  try {
    auto conn_guard = pool_.acquire();
    pqxx::work tx(conn_guard.get());
...
std::vector<DatabaseProcessor::PendingTask>
DatabaseProcessor::get_ready_for_retry() {
  ...
  try {
    ...
    pqxx::result r = tx.exec(
        "SELECT id, user_id, raw_input FROM pending_requests "
        "WHERE status != 'COMPLETED' AND next_retry_at <= CURRENT_TIMESTAMP "
        "ORDER BY next_retry_at ASC LIMIT 10");

    for (auto const &row : r) {
      tasks.push_back({row[0].as<int64_t>(), row[1].as<int64_t>(),
                       row[2].as<std::string>()});
    }
  } catch (const std::exception &e) {
