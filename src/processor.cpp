...
std::optional<Expense> Processor::process_message(std::string_view raw_content,
  ...
  try {
    ...
    if (extracted.is_array()) {
      if (extracted.empty()) throw std::runtime_error("Empty Gemini response");
    ...
    // If Gemini is busy or down (429, 503, Timeout), schedule a persistent retry
    if (err.find("429") != std::string::npos || 
        err.find("503") != std::string::npos || 
        err.find("timeout") != std::string::npos) {
        
        db_.schedule_retry(request_id, 5, err);
        std::cout << "[RETRY] API busy. Scheduled persistent retry for " << request_id << " in 5m" << std::endl;
    } else {
...
void Processor::retry_background_tasks() {
    ...
    for (const auto& task : tasks) {
        try {
            ...
            if (extracted.is_array() && !extracted.empty()) extracted = extracted[0];
            ...
            std::cerr << "[RETRY] Task " << task.id << " failed again: " << err << ". Postponed by " << delay << "m" << std::endl;
        }
    }
}
