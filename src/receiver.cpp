...
Receiver::Receiver(dpp::cluster &bot, Processor &processor, WorkerPool &workers,
                   std::optional<std::string> guild_id)
    : bot_(bot), processor_(processor), workers_(workers),
      guild_id_(std::move(guild_id)) {}
    ...
    workers_.enqueue([this, event, input, user_id]() {
      ...
      if (result.has_value()) {
        dpp::message msg(event.command.channel_id,
                         Replier::create_expense_embed(result.value()));
        event.edit_response(msg);
      } else {
        event.edit_response(
            "❌ Failed to parse or save expense. Please try again with more "
            "detail.");
      }
    });
  });
...
void Receiver::register_commands() {
  if (dpp::run_once<struct register_bot_commands>()) {
    ...
    exp_cmd.add_option(dpp::command_option(dpp::co_string, "query",
                                           "What did you spend?", true));
